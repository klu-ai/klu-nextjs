...
function Saved({
  ...
  const [selectedActionResponses, setSelectedActionResponses] =
    useInitialChange(
      storedActionResponses?.filter(
        (r) => r.actionGuid === selectedActionGuid
      ) ?? [],
      [selectedActionGuid, selectedActionGuid]
    )
  ...
  if (!storedActionResponses || selectedActionResponses.length === 0)
    return (
